...
def filter():
    ...
    for i in range(n):
        radio_button = ttk.Radiobutton(root, text=text[i], variable=vak, value=var[i], command=on_radiobutton_click) 
        radio_button.pack()
...
def on_radiobutton_click():
    ...
    if n == 1:
        messagebox.showinfo("GUI Python", f"Только одна собака соответствует фильтру:\n{filtered[0]['название']}")
    else:
        print("\n"*2)

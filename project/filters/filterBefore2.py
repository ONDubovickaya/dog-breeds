...
def filter():
    ...
    for i in range(n):
        checkbox = ttk.Checkbutton(root, text=text[i], variable=var2[i], command=on_checkbox_click2) 
        checkbox.pack()
    
    result_button = ttk.Button(root, text="Итог фильтрации", command=show_result2)
    result_button.pack()
...
def show_result2():
    ...
    if m == 0:
        ...
        #ind = [0]*m
        for i in range(m):
            data_for_sv[i] = [filtered[selected_vars[i]]['id'] - 1, str(filtered[selected_vars[i]]['название'])]

...
def comboboxes(event):
    # Очищаем предыдущие Radiobuttons, если они есть
    for widget in root.pack_slaves():
        if isinstance(widget, ttk.Checkbutton):
            widget.destroy()
        if isinstance(widget, ttk.Button):
            if widget["text"] == "Итог фильтрации":
                widget.destroy()
        if isinstance(widget, ttk.Label):
            if widget["text"] == "Выберите породы собак, которые вам нравятся (не менее одной):" or widget["text"] == "Выберите породы собак, которые вам НЕ нравятся (можно не выбирать):":
                widget.destroy()
            #print('destroy')
...
def filter():
    ...
    label31 = ttk.Label(root, text="Выберите породы собак, которые вам нравятся (не менее одной):")
    label31.pack()

    text = [0]*n
    for i in range(n):
        text[i]=filtered[i]['название']
        #var31[i] = IntVar()
    ...
    label32 = ttk.Label(root, text="Выберите породы собак, которые вам НЕ нравятся (можно не выбирать):")
    label32.pack()
    ...
    result_button3 = ttk.Button(root, text="Итог фильтрации", command=show_result3)
    result_button3.pack()
...
def show_result3():
    ...
    if m_like == 0:
        ...
        for i in range(m_like):
            data_for_sv_like[i] = [filtered[selected_vars_like[i]]['id'] - 1, str(filtered[selected_vars_like[i]]['название'])]
        print("Вам нравятся: ", data_for_sv_like)
        
        data_for_sv_dislike = [0]*m_dislike
        #ind = [0]*m_like
        for i in range(m_dislike):
            data_for_sv_dislike[i] = [filtered[selected_vars_dislike[i]]['id'] - 1, str(filtered[selected_vars_dislike[i]]['название'])]
